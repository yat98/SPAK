...
class SuratDispensasiRequest extends FormRequest
{
    ...
    public function rules()
    {
        if($this->method() == 'PATCH' || $this->method() == 'PUT'){
            $nomorSuratRules = 'required|numeric|min:1|unique:surat_dispensasi,nomor_surat,'.$this->get('id').',id_surat_masuk';
            $idSuratMasukRules = 'required|numeric|unique:surat_dispensasi,id_surat_masuk,'.$this->get('id').',id_surat_masuk';
        }else{
            $nomorSuratRules = 'required|numeric|min:1|unique:surat_dispensasi,nomor_surat';
