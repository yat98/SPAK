                            ...
                            <div class="card-body">
                                <div class="row mb-3">
                                    <div class="col-12 col-md-6">
                                        <h4>Pengajuan Surat Tugas</h4>
                                    </div>
                                    @if(Auth::user()->bagian == 'front office')
                                    <div class="col-12 col-md-6 text-right">
                                        <a href="{{ url('operator/pengajuan/surat-tugas/create')}}"
                                ...
                                <div class="row">
                                    <div class="col text-center">
                                        ...
                                        <p class="text-muted">
                                            {{ (Session::has('search')) ? Session::get('search') : ' Pengajuan surat tugas belum ada.' }}
                                        </p>
                                    </div>
                                </div>
                                @endif
...
@section('datatables-javascript')
    <script>
        let link = "{{ url('operator/pengajuan/surat-tugas') }}";
        let linkPengajuan = "{{ url('operator/surat-tugas/pengajuan') }}";
        let linkSurat = "{{ url('operator/surat-tugas') }}";
        let linkMhs = "{{ url('operator/detail/mahasiswa') }}";
        let linkOperator = "{{ url('operator/surat-tugas') }}";
        ...
        $('#datatables').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{{ url('operator/pengajuan/surat-tugas/all') }}',
            columns: [{
                    data: 'nama_kegiatan',
                },
                {
                            ...
                            "render": function ( data, type, row, meta ) {
                                <?php if(Auth::user()->bagian != 'front office'){ ?>
                                    return `<div class="d-inline-block">
                                                ...
                                                <div class="dropdown-menu navbar-dropdown border border-dark" aria-labelledby="aksi">
                                                    <a href="${linkOperator+'/'+row.id}" class="dropdown-item btn-surat-tugas-detail" data-toggle="modal" data-target="#suratTugas">Detail</a>
                                                    <a href="${link+'/'+row.id+'/cetak'}" class="dropdown-item">Cetak</a>
                                                </div>
                                            </div>`;
                                <?php }else{ ?>
                                    let action = `<a href="${linkOperator+'/'+row.id}" class="dropdown-item btn-surat-tugas-detail" data-toggle="modal" data-target="#suratTugas">Detail</a>`;
